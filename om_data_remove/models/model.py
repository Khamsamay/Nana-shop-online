...
class ResConfigSettings(models.TransientModel):
    ...
    def remove_data(self, o, s=[]):
        for line in o:
            ...
            self._cr.execute(sql, (t_name,))

            # Fetch the result
            result_company = self._cr.fetchall()

            # Process the result

            # Check context to prevent issues if 'allowed_company_ids' is missing or empty
            allowed_company_ids = self.env.company.id

            if allowed_company_ids and result_company:
                sql = "DELETE FROM %s WHERE company_id = %s" % (t_name, allowed_company_ids)
    ...
    def remove_purchase_request(self):
        to_removes = [
    ...
    def remove_account_chart(self):
        ...
        try:
            ...
            if allowed_company_ids:
                # Ensure that the allowed_company_ids list is properly formatted
                    # If there's only one element, just use that value directly
                sql = "delete from ir_default where (field_id = %s or field_id = %s) and company_id = %s" \
                % (field1, field2, allowed_company_ids)
                sql2 = "update account_journal set bank_account_id=NULL where company_id = %s;" % (allowed_company_ids)
        ...
        if self.env['ir.model']._get('pos.config'):
            rec = self.env['pos.config'].search([('company_id', 'in', [self.env.company.id])])
            if rec:
                rec.write({
                    'journal_id': False,
                })
