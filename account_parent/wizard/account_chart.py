...
class OpenAccountChart(models.TransientModel):
	...
	report_type = fields.Selection([('account', 'CoA based on Accounts'),
	...
	def build_domain_context(self, wiz_id=None, account_id=None):
		...
		if account_id:
			...
			query_init = self.env['account.move.line']._where_calc(domain=domain_initial)
			query_str, params = query_init.select('account_move_line.id')
	...
	def line_data(self, level, parent_id, wiz_id=False, account=False):
		return {
			...
			'name': account.name,
			'ac_type': self._m2o_to_str(account),
			'type': account.account_type,
			'currency': self._m2o_to_str(account.currency_id),
			'company': self._m2o_to_str(account.company_ids),
			'debit': self._float_html_formating(account.debit, account.company_ids),
			'credit': self._float_html_formating(account.credit, account.company_ids),
			'balance': self._float_html_formating(account.balance, account.company_ids),
			'company_obj':account.company_ids,
			'show_initial_balance': self._context.get('show_initial_balance',False),
			'initial_balance': self._float_html_formating(account.initial_balance, account.company_ids),
			# 'initial_balance': account.initial_balance,
			'ending_balance': self._float_html_formating(account.initial_balance + account.balance, account.company_ids),
			'db' : account.debit,
			'cr' : account.credit,
			'bal' : account.balance,
	...
	def account_type_data(self):
		...
		for parent_account_type in parent_account_types_temp:
			if not parent_account_type['parent_id']:
				continue
			account_types = self.env['account.account'].search([
	...
	def get_lines(self, wiz_id=None, line_id=None, **kw):
		...
		res = getattr(self, 'get_%s_lines' % report_type)(wiz_id, line_id, level)
...
# 		lines = self.final_vals_to_lines(final_vals, level, html_formating)
	...
	def _get_html(self):
		...
		# result['html'] = self.env['ir.qweb']._render('account_parent.report_coa_hierarchy', rcontext)
		# print('0000eww0----------', type(self.env['ir.qweb']._render('account_parent.report_coa_hierarchy', rcontext)) )

		# rendered_html = self.env['ir.qweb']._render('account_parent.report_coa_hierarchy', rcontext)
